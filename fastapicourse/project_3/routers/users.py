...
async def update_password(
    user: UserDep, password_request: types.PasswordRequest, db: SessionDep
):
    # Validate that the user exists and that the old password is correct
    if user is None or not types.bcrypt_context.verify(
        password_request.old_password, user.hashed_password
    ):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Could not validate credentials",
...
@router.put(
    "/update/phone_number/{phone_number}", status_code=status.HTTP_204_NO_CONTENT
)
async def update_phone_number(
    user: UserDep, phone_number: types.PhoneNumberType, db: SessionDep
):
